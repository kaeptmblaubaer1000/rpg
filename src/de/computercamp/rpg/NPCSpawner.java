...
public class NPCSpawner {
	public static void spawnRandomNPCs(Map map, Vector2D minPos, Vector2D maxPos) {
		...
		NPC magician = new ItemGivingNPC(getRandomLocation(minPos, maxPos), 1, new HealingPotion(new Vector2D(0,0)), (long) -1);
		NPC badmagician = new HealthChangingNPC(getRandomLocation(minPos, maxPos), 2, -6, 8, 20000);
		NPC weaponsmith = new ItemGivingNPC(getRandomLocation(minPos, maxPos), 3, new Sword(new Vector2D(0,0)), (long) -1);
